...
def main():
    ...
    if st.button("🚀 Detect Outliers"):
        ...
        y_test_visual = np.zeros_like(y_pred)

        # Confusion Matrix
        st.subheader(f"📊 Confusion Matrix - {model_name}")
        fig_cm, ax = plt.subplots()
        cm = confusion_matrix(y_test_visual, y_pred)
        disp = ConfusionMatrixDisplay(cm, display_labels=["Inlier", "Outlier"])
        disp.plot(ax=ax, cmap="Blues")
        st.pyplot(fig_cm)
        st.success(f"✅ Detection consistency: {(y_pred == y_test_visual).mean():.2f}")

        # Dimensionality Reduction Visualization
        for method in dim_methods:
            st.subheader(f"🧭 {method} Projection ({n_components}D)")
            X_reduced = reduce_dimensions(X_test, method, n_components)

            if n_components == 2:
                fig, ax = plt.subplots(figsize=(6, 5))
                ax.scatter(X_reduced[:, 0], X_reduced[:, 1], c="blue", label="Inliers", alpha=0.7)
                ax.scatter(X_reduced[y_pred == 1, 0], X_reduced[y_pred == 1, 1], c="red", label="Outliers")
